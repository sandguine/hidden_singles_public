...
df = df.sim.strategy %>% 
  filter(strategy == 'HS') %>% 
  crossing(threshold = c(.5, .667, .8, .9)) %>% 
  filter(p_predicted >= threshold) %>% 
...
df %>% 
  ggplot(aes(x = trial, y = accuracy, color = threshold)) +
  geom_line() +
  geom_point() + 
  labs(x = "Trial",
       y = "Accuracy") +
  guides(color = guide_legend(title="Threshold")) +
  theme(legend.position = c(.89, .245),
        legend.box.background = element_rect(colour = "black")) +
  ggsave(glue("../figures/practice_phase/p_hs_accuracy.png"), width=8, height=4.5)
```
...
df.xsim.actual %>% 
  ...
  ggsave(glue("../figures/practice_phase/pred_xtrial.png"), width=8, height=4.5)
```
...
df.auc %>% 
  ...
  theme(legend.position = c(.85, .21),
        legend.box.background = element_rect(colour = "black")) +
  ggsave(glue("../figures/practice_phase/auc.png"), width=8, height=4.5)
```
...
df_puzzle %>% 
  ...
  theme(legend.position = c(.86, .805),
        legend.box.background = element_rect(colour = "black")) +
  ggsave(glue("../figures/practice_phase/responses.png"), width=8, height=4.5)
```
...
df_p_responses %>% 
  ...
  theme(legend.position = c(.86, .805),
        legend.box.background = element_rect(colour = "black")) +
  ggsave(glue("../figures/practice_phase/p_responses.png"), width=8, height=4.5)
```
...
df_p_strategies %>% 
  ...
  theme(legend.position = c(.89, .24),
        legend.box.background = element_rect(colour = "black")) +
  ggsave(glue("../figures/practice_phase/p_strategies.png"), width=8, height=4.5)
```
...
```{r}
df %>% 
  ggplot(aes(x = trial, y = p_hs, color = Threshold)) +
  geom_line() +
  geom_point() +
  labs(title = TeX("P(successful) $\\geq$ Threshold"),
       x = 'Trial',
       y = '% of Participants') +
  theme(legend.position = c(.88, .245),
        legend.box.background = element_rect(colour = "black")) +
  ggsave(glue("../figures/practice_phase/p_hs.png"), width=8, height=4.5)
```
