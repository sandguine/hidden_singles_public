...
ggplot() +
  ...
  #       legend.position=c(.85, .899),
  #       legend.box = "horizontal") +
  # guides(fill = guide_legend(ncol = 2, nrow = 1)) +
  ggsave(glue("../figures/questionnaire/q_mc.png"), width=8, height=6)
```
...
df.data %>% 
  ...
  ggsave(glue("../figures/questionnaire/q_pd.png"), width=8, height=6)
```
...
df.data %>% 
  ...
  labs(title = 'Basis for Choice Classifications',
       ...
       y = 'Rater 2') +
  guides(fill = guide_colorbar(title = "No. of Subjects")) +
  ggsave(glue("../figures/questionnaire/q_basis.png"), width=8, height=6)
```
...
df = df.data %>% 
  ...
  group_by(solver) %>% 
  summarize(x = sum(valid_basis),
            n = n())

bayes.prop.test(df$x, df$n) %>% 
  plot()
```
...
df = df.data %>% 
  filter(correct_actual) %>% 
  mutate(solver = solver == 'Solver') %>% 
  group_by(basis) %>% 
  summarize(x = sum(solver),
            n = n())
...
df.sum.stats = get_counts_with_zeros(df, c('solver', 'basis')) %>% 
  left_join(get_dirichlet_hdci(df, c('solver', 'basis'), 100000)) %>% 
...
df %>% 
  ...
  ggplot(aes(x = category, fill = solver, group = solver)) +
  ...
  labs(x = "Basis",
       y = "% of Participants") +
  ggsave(glue("../figures/questionnaire/q_ratings.png"), width=8, height=6)
```
...
df.sum.stats = model %>% 
  ...
  mutate(edu_level = case_when(edu_level == 'Associates' ~ "Associate's",
...
df.edu %>% 
  ...
  theme(legend.position = "bottom") +
  ggsave(glue("../figures/questionnaire/education.png"), width=8, height=6)
```
